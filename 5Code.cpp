...
static void PlatformMouseClick() {
    INPUT inputs[2] = {};
...
static void CmdMouse(std::vector<std::string>& args, bool dryRun) {
    if (args.empty() || ToLower(args[0]) != "click") { Log("[ERROR] 사용법: Mouse(\"Click\")", 3); return; }
    if (dryRun) { Log("[DRY-RUN] 마우스 클릭 예정", 2); return; }
    try { PlatformMouseClick(); Log("완료: 마우스 클릭됨", 1); }
    catch (std::exception& e) { Log(std::string("[ERROR] ") + e.what(), 3); }
}
...
static void CmdHelp() {
    Log(
        ...
        "[자동입력] Wait(ms) MouseMoveX(n) MouseMoveY(n) Mouse(\"Click\") Keyboard(\"key\")\n"
...
static void CmdVersion() { Log("jat Terminal v4.0.0 (Cross-Platform CLI: Windows + Linux, C++17)", 1); }
