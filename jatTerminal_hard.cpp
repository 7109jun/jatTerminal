...
// HARD 모드: Assert/Confirm 실패 시 Batch/Autowork 시퀀스를 중단시키기 위한 협력적 플래그
...
static std::ofstream g_logMirror;      // Log2File 토글용
static bool g_logMirrorOn = false;
static std::string* g_captureBuffer = nullptr; // Pipe 캡처용 (null이면 일반 출력)
...
static void Log(const std::string& text, int colorType) {
    ...
    while (std::getline(iss, line)) {
        if (g_captureBuffer) { *g_captureBuffer += line + "\n"; }
        else {
            std::cout << col << line << C_RESET << "\n";
            if (g_logMirrorOn && g_logMirror) g_logMirror << line << "\n";
        }
        any = true;
    }
    if (!any && !g_captureBuffer) std::cout << "\n";
    if (g_logMirrorOn && g_logMirror) g_logMirror.flush();
}
...
static void CmdBatch(std::vector<std::string>& args, bool dryRun) {
    ...
    for (auto& s : steps) {
        auto t = Trim(s); if (t.empty()) continue;
        ExecuteLine(t);
        if (g_abortFlag) { g_abortFlag = false; Log("[중단] Batch 실행이 Assert/Confirm에 의해 중단됨", 3); return; }
    }
...
static void CmdExists(std::vector<std::string>& args, bool dryRun) {
    if (args.empty()) { Log("[ERROR] 사용법: Exists(\"경로\")", 3); return; }
    if (dryRun) { Log("[DRY-RUN] 존재 여부 확인 예정: " + args[0], 2); return; }
    bool ex = fs::exists(fs::u8path(args[0]));
    Log(ex ? ("존재함: " + args[0]) : ("존재하지 않음: " + args[0]), 1);
}
...
static void HandleAutoworkLine(const std::string& line, bool dryRun) {
    ...
    try {
        AutoworkParser parser(body); parser.runStatements();
        if (g_abortFlag) { g_abortFlag = false; Log("[중단] Autowork 실행이 Assert/Confirm에 의해 중단됨", 3); }
        else Log("완료: Autowork 실행됨", 1);
    }
...
static void CmdHelp() {
    Log(
        ...
        "[편의] Cd Path Exists Timestamp Env Random Count Sort Merge WordCount\n"
...
static void ExecuteParsed(const ParsedCommand& pc) {
    ...
    try {
        ...
        else if (nl=="exists") CmdExists(args, pc.dryRun);
