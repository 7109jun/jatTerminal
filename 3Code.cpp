// ============================================================================
// jat Terminal v3.0.0이다!!!!!!!!!!!!!!!
// jat명령어 터미널 시스템 - 진짜 CLI (Windows + Linux 지원)
// 초록(강조/프롬프트) / 검정(배경, 터미널 기본) / 회색(보조텍스트) / 흰색(본문)
// 의존성: curl (Download/Upload/API/Ping 보조), 파이썬3(리눅스)파이썬(윈도우) (^코드^ 실행하다)
// ============================================================================
#ifdef _WIN32
...
static void CmdHelp() {
    Log(
        ...
        "※ Download/Upload/API/Ping은 시스템 curl/ping 명령을 사용합니다.",
        1);
}
static void CmdVersion() { Log("jat Terminal v3.0.0 (Cross-Platform CLI: Windows + Linux, C++17)", 1); }
...
static void ExecuteLine(const std::string& raw) {
    ...
    if (!pc.valid) { Log("[ERROR] " + pc.error, 3); return; }
...
int main() {
    ...
    PurgeOldQuarantine();
    PrintBanner();
    Log("jat Terminal v3.0.0 - 준비 완료. help() 를 입력하세요.", 1);
